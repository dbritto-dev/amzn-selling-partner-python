...
## 14. Build-time generation with oagen (supersedes the runtime-loading design)
...
### 14.1 What oagen does and does not give us (measured on the pinned models)
...
```
codegen/                       Node 22 project (TypeScript, tsx)
  package.json                 @workos/oagen 0.30.2, swagger2openapi
  oagen.config.ts              emitter registration + transformSpec (CLI use)
  src/generate.ts              driver: every model file + notification schema
  src/convert.ts               Swagger 2.0 → OpenAPI 3.0 (+ known-bad $ref fixes)
  src/transform.ts             alias inlining, name protection
  src/amazon.ts                api naming, aliases, pagination overrides, RDT-free
  src/emitter/                 the Python emitter (models, resources, apis, naming, types, pagination, ratelimits)
...
Removed: `spec/`, `compile/`, `stubgen.py`, `stubs/`, the IR pickle cache,
`scripts/sync_specs.py`, `scripts/spec_naming.py`, the `Client(specs=...)`
runtime loader and the `enum_mode="plain"` option (enums are always `Literal`).
The wheel ships only Python; the spec submodule is a generator input.

### 14.3 Generated code shape
...
### 14.5 Measured (this branch, Python 3.12)

| | runtime-loaded (before) | generated (after) |
|---|---|---|
