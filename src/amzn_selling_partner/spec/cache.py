...
directory defaults to ``$AMZN_SELLING_PARTNER_CACHE_DIR`` or ``~/.cache/amzn_selling_partner/ir``; set
``AMZN_SELLING_PARTNER_CACHE_DIR=`` (empty) to disable caching.
"""
...
def cache_dir() -> pathlib.Path | None:
    env = os.environ.get("AMZN_SELLING_PARTNER_CACHE_DIR")
    if env is not None:
        return pathlib.Path(env) if env else None
    return pathlib.Path.home() / ".cache" / "amzn_selling_partner" / "ir"
