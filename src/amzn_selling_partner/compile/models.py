...
* Recursion: dependencies are built depth-first. A back-edge to a schema that is
  ...
  registered in a real module (``amzn_selling_partner.models.<key>``) so pydantic resolves those
...
log = logging.getLogger("amzn_selling_partner.compile.models")
...
def _module_for(key: str) -> types.ModuleType:
    name = f"amzn_selling_partner.models.{key}"
