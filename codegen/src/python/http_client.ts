/**
 * `http_client.py`: the HTTP layer every generated resource calls. Retry,
 * backoff and timeout policy come from `ctx.spec.sdk` (oagen defaults merged
...
export function renderHttpClientModule(ctx: EmitterContext, opts: EmitterOptions): string {
  ...
  const timeoutExpr = timeoutEnv
    ? `float(os.environ.get(${pyStr(timeoutEnv)}, ${sdk.timeout.defaultTimeoutSeconds}))`
    : String(sdk.timeout.defaultTimeoutSeconds);
  const rateHint = opts.rateHintHeader ? pyStr(opts.rateHintHeader) : 'None';
  const errorClasses = ['APIConnectionError', 'APIResponseValidationError', 'APIStatusError', 'APITimeoutError', 'status_error_class'];
  return `"""HTTP client of the ${ctx.spec.name} SDK.

Retry, backoff and timeout policy come from the spec's SDK behavior
(\`\`sdkBehavior\`\` in oagen.config.ts). Every generated resource method builds
its parameters and calls \`\`HttpClient.request\`\` / \`\`AsyncHttpClient.request\`\`.
...
from .errors import ${errorClasses.join(', ')}
from .models._base import adapter_for

if TYPE_CHECKING:
    from typing_extensions import Self

log = logging.getLogger(${pyStr(ctx.namespace.toLowerCase() + '.http_client')})
